...
# This code is used to compute spatial derivatives using a convolutional filter
...
def ddx(inpt, channel, dx, scope='ddx', name=None):
    # Compute x derivative using a convolutional filter
...
def ddy(inpt, channel, dy, scope='ddy', name=None):
    # Compute y derivative using a convolutional filter
    inpt_shape = inpt.get_shape().as_list()
    var = tf.expand_dims( inpt, axis=4)
...
def ddz(inpt, channel, dz, scope='ddz', name=None):
    # Compute z derivative using a convolutional filter
    inpt_shape = inpt.get_shape().as_list()
    var = tf.expand_dims( inpt, axis=4)
